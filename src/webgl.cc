...
v8::Handle<v8::Value> ThrowError(const char* msg) {
  return v8::Isolate::GetCurrent()->ThrowException(v8::Exception::Error(v8::String::NewFromUtf8(v8::Isolate::GetCurrent(),msg)));
}
...
inline void *getImageData(Local<Value> arg) {
  ...
  if (!arg->IsNull()) {
    ...
    if (!obj->IsObject())
      v8::Isolate::GetCurrent()->ThrowException(JS_STR("Bad texture argument"));
...
inline Type* getArrayData(Local<Value> arg, int* num = NULL) {
  ...
  if(!arg->IsNull()) {
    if(arg->IsArray()) {
      ...
      v8::Isolate::GetCurrent()->ThrowException(JS_STR("Bad array argument"));
  }
...
NAN_METHOD(GetError) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),glGetError()));
}
...
NAN_METHOD(GetAttribLocation) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),glGetAttribLocation(program, *name)));
}
...
NAN_METHOD(CreateShader) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),shader));
}
...
NAN_METHOD(GetShaderInfoLog) {
  ...
  NanReturnValue(String::NewFromUtf8(v8::Isolate::GetCurrent(),Error));
}
...
NAN_METHOD(CreateProgram) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),program));
}
...
NAN_METHOD(CreateTexture) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),texture));
}
...
NAN_METHOD(CreateBuffer) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),buffer));
}
...
NAN_METHOD(CreateRenderbuffer) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),renderbuffers));
}
...
NAN_METHOD(IsBuffer) {
  ...
  NanReturnValue(Boolean::New(v8::Isolate::GetCurrent(),glIsBuffer(args[0]->Uint32Value())!=0));
}
...
NAN_METHOD(GetShaderSource) {
  ...
  Local<String> str=String::NewFromUtf8(v8::Isolate::GetCurrent(),source);
  delete source;
...
NAN_METHOD(GetTexParameter) {
  ...
  NanReturnValue(Number::New(v8::Isolate::GetCurrent(),param_value));
}
...
NAN_METHOD(GetActiveAttrib) {
  ...
  Local<Array> activeInfo = Array::New(v8::Isolate::GetCurrent(),3);
...
NAN_METHOD(GetAttachedShaders) {
  ...
  Local<Array> shadersArr = Array::New(v8::Isolate::GetCurrent(),count);
...
NAN_METHOD(GetParameter) {
  ...
  case GL_VIEWPORT:
  {
    ...
    Local<Array> arr=Array::New(v8::Isolate::GetCurrent(),4);
    arr->Set(0,JS_INT(params[0]));
    arr->Set(1,JS_INT(params[1]));
...
NAN_METHOD(GetUniform) {
  ...
  Local<Array> arr=Array::New(v8::Isolate::GetCurrent(),16);
  for(int i=0;i<16;i++)
...
NAN_METHOD(GetExtension) {
  ...
  NanReturnValue(JS_STR(ext, v8::String::kNormalString,(int)::strlen(sname)));
}
